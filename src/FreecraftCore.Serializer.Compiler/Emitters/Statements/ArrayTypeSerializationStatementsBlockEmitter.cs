...
namespace FreecraftCore.Serializer
{
	public sealed class ArrayTypeSerializationStatementsBlockEmitter : BaseSerializationStatementsBlockEmitter
	{
		...
		public override List<StatementSyntax> CreateStatements()
		{
			...
			{
				var generator = new ArraySerializerGenerator(Member.Name, new SendSizePrimitiveArrayInvokationExpressionEmitter(ActualType.GetElementType(), sendSizeAttri.TypeOfSize));
				statements.Add(generator.Create());
			}

			if (knownSizeAttri != null)
			{
				var generator = new ArraySerializerGenerator(Member.Name, new FixedSizePrimitiveArrayInvokationExpressionEmitter(ActualType.GetElementType(), knownSizeAttri.KnownSize));
				statements.Add(generator.Create());
			}
			...
			{
				var generator = new ArraySerializerGenerator(Member.Name, new DefaultPrimitiveArrayInvokationExpressionEmitter(ActualType.GetElementType()));
				statements.Add(generator.Create());
			}
